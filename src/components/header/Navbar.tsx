...
const Navbar = () => {
  ...
  return (
    <div className="sticky top-0 z-50 w-full backdrop-blur-md bg-white/60 dark:bg-zinc-900/60 border-b border-zinc-100/70 dark:border-zinc-800/60 supports-[backdrop-filter]:bg-white/40 shadow-sm">
      <nav className="relative select-none lg:flex lg:items-stretch w-full max-w-7xl mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          <Link
            href="/"
            className="text-xl font-bold bg-gradient-to-r from-purple-500/80 to-blue-400/80 bg-clip-text text-transparent hover:from-purple-400 hover:to-blue-300 transition-colors drop-shadow-sm"
          >
            PILPUT
          </Link>
        ...
        <div
          className={cn(
            ...
            showmenu
              ? "absolute top-16 left-0 right-0 bg-white/90 dark:bg-zinc-900/90 border-b border-zinc-100/70 dark:border-zinc-800/60 shadow-md rounded-b-xl lg:border-none"
              : "hidden lg:flex"
          )}
        >
            ...
            {navigation.map((item) => (
              <Link
                ...
                className={cn(
                  ...
                  pathname === item.href
                    ? "text-purple-500 dark:text-purple-300 bg-purple-50/70 dark:bg-purple-900/30 shadow-sm"
