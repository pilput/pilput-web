      ...
      {isLoading ? (
        <div className="grid gap-3 sm:gap-4 grid-cols-2 lg:grid-cols-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <Card key={i} className="animate-pulse overflow-hidden glass-card border-glow-hover rounded-2xl">
              <div className="h-24 sm:h-32" />
            </Card>
          ))}
        </div>
      ) : error ? (
        <Card className="glass-card border-red-500/20 bg-red-500/5 dark:bg-red-950/20 dark:border-red-900 rounded-2xl overflow-hidden transition-all duration-300">
        ...
        <Card className="glass-card border-dashed border-muted bg-muted/5 rounded-2xl overflow-hidden transition-all duration-300">
