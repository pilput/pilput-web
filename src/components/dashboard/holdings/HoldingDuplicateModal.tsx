...
const getDefaultFormData = (): HoldingDuplicateFormData => {
  const now = new Date();
...
function HoldingDuplicateModalForm({
  onDuplicate,
  onCancel,
}: {
  onDuplicate: HoldingDuplicateModalProps["onDuplicate"];
  onCancel: () => void;
}) {
  const [formData, setFormData] = useState<HoldingDuplicateFormData>(() =>
    getDefaultFormData()
  );
